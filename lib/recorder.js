...
class Recorder {
  ...
  _registerIpc() {
    ipcMain.on(channels.SEGMENT_DATA, (event, { index, startTs, endTs, bytes }) => {
      ...
      const segment = { path: filePath, startTs, endTs };
      this._segments.push(segment);
      for (const fn of this._segmentHooks) {
        try {
          fn(segment);
        } catch (err) {
          console.error('[recorder] segment hook failed:', err.message);
        }
      }
    });
    ...
    ipcMain.on(channels.CAPTURE_STOPPED, async () => {
      ...
      try {
        ...
        // Give consumers (e.g. Module 3's highlight marker) a chance to finish
        // reading segment files before they're deleted below.
        await Promise.all(this._preClearHooks.map((fn) => Promise.resolve().then(fn)));
        this._clearBuffer();
  ...
  // Registers a function to be awaited after stitching but before the
  // segment buffer is cleared, so in-flight consumers of getSegments()
  // (e.g. the highlight marker) never race the cleanup.
  registerPreClearHook(fn) {
    this._preClearHooks.push(fn);
  }

  // Registers a function called synchronously each time a segment finishes
  // writing to the buffer (e.g. Module 4's auto-detect engine).
  registerSegmentHook(fn) {
    this._segmentHooks.push(fn);
  }
