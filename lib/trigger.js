...
const { buildAutoDetectMenuItem } = require('./auto-detect/tray-menu');
...
class Trigger {
  constructor(recorder, config, marker, autoDetect) {
    this.recorder = recorder;
    this.config = config;
    this.marker = marker || null;
    this.autoDetect = autoDetect || null;
  ...
  _rebuildTrayMenu() {
    ...
    if (this.autoDetect) {
      template.push(
        buildAutoDetectMenuItem(this.autoDetect, { onToggle: () => this._rebuildTrayMenu() })
      );
    }
